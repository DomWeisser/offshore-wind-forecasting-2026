...
## Paper
The paper attached to this respository can be found here: https://arxiv.org/abs/2601.19674
...
To facilitate the reproduction of results, the data required for this analysis (notebooks 2 and 3) have been included in this repository under "Data". These files contains hourly records for the selected source and target wind farms, filtered to the last 5 years of available data (01.01.2015 - 31.12.2019). Only the last 2 years was used to achieve the results in the paper.
...
This notebook implements the transfer learning methodology across the eight target wind farms (Horns Rev, Seagreen, Gemini, Hollandse Noord, Dieppe, Moray Firth, Kriegers Flak and East Anglia One). Additionally, it includes the code to run three comparative baselines, namely target farm-level GP and MLP models as well as cluster-specific GPs trained from scratch (no fine-tuning from source models), to benchmark the transfer performance of the proposed framework. To facilitate reproducibility, the requisite metadata and pre-trained VAE artificats needed in the transfer pipeline for all three seeds are available in the models directory. 

### Results 
A breakdown of the results for all three random seeds (42, 63, 84) used in the final paper can be found in "Results Breakdown.xslx". This file contains the source GP results as well as the transfer learning (and baselines) forecasting performance for all eight target farms, stratified by cluster and data capacity.
